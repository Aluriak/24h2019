...
# class Laumio:
    ...
    def init_all(*args, **kwargs):
        for name_or_id in conf.SPATIAL_POSITION:
...
def laumio_from_name_or_id(id_or_name:str or int, context={}):
    # print(f'LAUMIO FROM {id_or_name} in {conf.SPATIAL_POSITION}')
    if id_or_name.isnumeric():
        id_or_name = int(id_or_name)
    if isinstance(id_or_name, int):
        id_or_name = conf.SPATIAL_POSITION[id_or_name]
...
METAMODEL = textx.metamodel_from_file(
    "grammar.tx", classes=model_class.classes, debug=False
)
